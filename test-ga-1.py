...
#
# test-ga-1.py : esegue un esempio utilizzando la libreria sugli algoritmi genetici
#
...
# add_path_to_lib(('genetic algorithms',),False)
...
def objective_test(individual):
	# la funzione obiettivo valuta la fitness dell'individuo rispetto all'obiettivo
	target = list('#@! ..this is an example on how to use the genetic algorithm and strings.. !@#')
	return fitness_test(individual,target)
	
def main(argv):			  
	# e.g.: py test-ga-1.py 1000 100 6 0.25 20 10
	#		py test-ga-1.py 1000 100 6 0.25 0 20
	randomize()
	try:
